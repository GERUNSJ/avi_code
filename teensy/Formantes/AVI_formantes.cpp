...
void obtener_formantes(float* x, int x_n, int p, int Fs, int* f1, int* f2)
{
  ...
  // Luego habrá que chequear si efectivamente se habían encontrado frqs/formantes o no
  // y ver que hacer en caso de que no.
  float formantes[3]; // Sí solo usamos 2...
  uint8_t ff = 0;	
  for(uint8_t i=0; i<pp; i++)
  {
  ...
  if (ff >= 2) // Si hay dos o más formantes, guardo los dos primeros como F1 y F2. Else, 0
  {
	  *f1 = formantes[0];
	  *f2 = formantes[1];
	  return;
